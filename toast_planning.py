...
def plan(start_state):
    # TODO: implement me!
    to_visit = [(start_state["time"], start_state, [])]
    visited = set()

    while len(to_visit) > 0:
        # Sort the queue based on least time
        to_visit.sort(key=lambda x: x[0])
        current_time, current_state, current_sequence = to_visit.pop(0)

        if goal(current_state) == True:
            return current_sequence
        # Mark the state as visited
        visited.add(tuple(current_state.items()))
        
        # Visit possible neighboring states
        for action in actions:
            new_state = state_transition(current_state, action)
            new_time = new_state["time"]
            # If the state hasn't been visited then add it
            if tuple(new_state.items()) not in visited:
                visited.add(tuple(new_state.items()))
                new_sequence = current_sequence + [action]
                to_visit.append((new_time, new_state, new_sequence))
    return []
